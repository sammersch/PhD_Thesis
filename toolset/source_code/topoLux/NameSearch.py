...
def SyntacticQuery(n): # displays all microtoponyms with a syntactic length n.
    # min = 1, max = 7
    results = []
...
def Odonyms(): # displays all Odonyms. For the moment incorporates names in 'rue', 'chemin', 'strooss' and 'strasse'.
    # The queries are standardized to miniscules.
    results = []
    for item in micro_dict:
        if item["microtoponym"].lower().startswith('rue'):
            results.append(item["microtoponym"])
        elif item["microtoponym"].lower().startswith('chemin'):
            results.append(item["microtoponym"])
        elif 'strooss' in item["microtoponym"].lower():
            results.append(item['microtoponym'])
        elif 'strasse' in item["microtoponym"].lower():
            results.append(item['microtoponym'])
    return results


def Odonyms_ctrl(topo_dict): # displays all Odonyms. For the moment incorporates names in 'rue', 'chemin', 'strooss' and 'strasse'.
    # The queries are standardized to miniscules.
    # control version which can specify which dictionary to access.
    results = []
    for item in topo_dict:
...
# print(simplex('Kiischbierg'))

for item in microTopoAll:
    if 'Déiregaart'.lower() in item['name']:
        print(item['name'])
#     if item['name'].split()[-1].endswith('mier'):
#         print(item)
